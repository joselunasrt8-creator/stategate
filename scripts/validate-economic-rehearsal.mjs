...
const report = {
  ...
  defects_discovered: [
    { id: 'RELEASE_PAYLOAD_MUTATED_BY_REHEARSAL_TEST_FIX', observation: 'The rehearsal changed test.mjs even though test.mjs is included in the immutable v1.1.1 release manifest; the final standalone release verification therefore rejected the branch.', preserved_result: 'node scripts/verify-release.mjs: runtime file hash differs: test.mjs' },
  ],
  corrections_made: [
    { id: 'RELEASE_PAYLOAD_MUTATED_BY_REHEARSAL_TEST_FIX', correction: 'Restore test.mjs byte-for-byte from the frozen starting commit and rely on CI checkout/history/identity setup for the existing Git topology fixtures.', scope: 'test.mjs restoration only; no StateGate runtime, protocol, or historical release provenance changed', rerun_expected: 'all conformance and standalone release verification checks PASS' },
  ],
